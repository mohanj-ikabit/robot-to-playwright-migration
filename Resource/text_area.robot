...
4_1_1_3_text_area_splchar_not_allowed
    [arguments]  ${ID}
    clear element text    xpath=//*[@id="${ID}"]
...
4_1_1_4_text_area_numbers_allowed
    [arguments]  ${ID}
    clear element text    xpath=//*[@id="${ID}"]
...
4_1_1_5_text_area_numbers_not_allowed
     [arguments]  ${ID}
     clear element text    xpath=//*[@id="${ID}"]
     ${PATTERN}=  SeleniumLibrary.Get Element Attribute     xpath=//*[@id="${ID}"]    attribute=pattern
     ${t}    Should Not Be Equal    ${PATTERN}   12345687890   
     Run Keyword If     ${t}==False    Log to Console  Text field does not allow number - ${ID}
     Run Keyword If     ${t}==True    Log to Console  Text field allows number - ${ID}
    
4_1_1_6_text_area_minlength
     [arguments]  ${ID}    ${MIN_LENGTH}
     #reload page
     Clear Element Text    xpath=//*[@id="${ID}"]
     SeleniumLibrary.Input Text    xpath=//*[@id="${ID}"]   ${MIN_LENGTH}
     ${p}=    Get Value    xpath=//*[@id="${ID}"]
     log to console    ${p}.....is accepted in....${ID}
 
4_1_1_7_text_area_maxlength
...
4_1_1_8_text_area_confirm_text
...
4_1_1_9_text_area_pastable
    [arguments]  ${ID}
    #reload page
    Input text   xpath=//*[@id="${ID}"]    23423^%sdsd
    Press Keys   xpath=//*[@id="${ID}"]    Ctrl+ A
    Press Keys   xpath=//*[@id="${ID}"]    Ctrl + c
    Press Keys   xpath=//*[@id="${ID}"]    Ctrl+ v
    ${e}    get value    xpath=//*[@id="${ID}"]
    ${f}    Should Contain    ${e}      23423^%sdsd
    Run Keyword If  ${f}== True     Log to Console    Text is pastable in the field - ${ID}
    Run Keyword If  ${f}== False     Log to Console    Text is pastable in the field - ${ID}


4_1_1_10_text_area_notpastable
    [arguments]  ${ID}
    #reload page
    Press Keys   xpath=//*[@id="${ID}"]    Ctrl+a 
    Press Keys   xpath=//*[@id="${ID}"]     Ctrl+c
    ...
    Run Keyword If  ${Status}== 1     Log to Console    Text is not pastable in the field - ${ID}
...
#text no alphabets
   ...
   # Log to Console     Text field does not allow alphabets - ${ID}
   


4_1_1_11_text_area_spacebefore
      [arguments]  ${ID}
      SeleniumLibrary.Input Text    xpath=//*[@id="${ID}"]    ${SPACE}text
      ${d}    Get Text    xpath=//*[@id="${ID}"]
      Should contain    ${d}    ${SPACE}text   
      log to console  ${ID}..........Text field does accept space before 
 

4_1_1_12_text_area_spaceafter
     [arguments]  ${ID}
     SeleniumLibrary.Input Text    xpath=//*[@id="${ID}"]    text${SPACE}
      ${d}    Get Text    xpath=//*[@id="${ID}"]
      Should contain    ${d}    text${SPACE}
      log to console  ${ID}..........Text field does accept space after
     
4_1_1_13_test_area_nosuggestions
	[arguments]  ${ID}
	Click element	xpath=//*[@id="${ID}"]
	Textfield should contain	xpath=//*[@id="${ID}"]	 ${EMPTY}
	log to console    ${ID} is not showing any suggestions
  
4_1_1_14_text_area_field_default_value
	[arguments]	${ID}   ${text}
	${pattern}=	Get text	xpath=//*[@id="${ID}"]
	should be equal as strings    ${text}   ${pattern}
	log to console    ${ID} has default value.........${text}

4_1_1_15_text_area_field_default_state_enabled
	[arguments]	${ID}
	Element should be enabled	xpath=//*[@id="${ID}"]
	log to console	${ID} is enabled

4_1_1_16_text_area_field_default_state_disabled
	[arguments]	${ID}
	Element should be disabled	xpath=//*[@id="${ID}"]
	log to console	${ID} is disabled

4_1_1_17_text_area_field_label
	[arguments]	${ID}   ${text}
	${pattern}=	Get text	xpath=//*[@id="${ID}"]
	should be equal as strings    ${text}   ${pattern}
	log to console    ${ID} has label.......${text}

4_1_1_18_text_area_field_space_allowed
    [arguments]	${ID}
...
4_1_1_19_text_area_field_space_not_allowed
    [arguments]	${ID}
    SeleniumLibrary.Input Text    xpath=//*[@id="${ID}"]    ${strspc}
    ${pattern}=	Get text	xpath=//*[@id="${ID}"]
